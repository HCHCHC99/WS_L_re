...
3. 用户在 Keil Watch 修改 `g_foc_angle_input`（x → y）→ 程序检测到变化
4. 磁场指向 y（`valpha=V·cosθ, vbeta=V·sinθ → Foc_Svpwm → SetDuty3Phase`），吸附 **2s**
...
| HOLD_ATTRACT | 4 | 磁场定 y，计时 2s | → HOLD_VERIFY |
...
- **电角度**，0~360，静止系（校准后 0° = 转子 N 极零位）；int32，代码内 wrap 到 [0,360)
- 渐进扫描（每步 ≤90°）：90°→mech 9°、180°→18°、270°→27°、0°/360°→36°，
  每步 +9° 机械（360°/10 对极），四步走完一个电周期 = 36° 机械
- **陷阱 1**：只对渐进扫描成立。跳变 >180° 电角度时转子走短路径
  （如 0° 直接设 270° → 转子倒退到 mech −9°=351°）
- **陷阱 2**：恰好 180° 跳变转矩为零（sin180°=0），死点，转子不动或乱摆，实验时避开
- 0° 与 360° 等价（同一磁场方向）
...
| 变量 | 类型 | 说明 |
|---|---|---|
| `g_foc_angle_input` | volatile int32_t | 用户输入电角度 deg（Watch 写） |
...
| `g_calang_target_deg` | volatile int32_t | 快照：锁存的目标角 |
| `g_calang_meas_deg` | volatile int32_t | 快照：校验结束时编码器实测电角度（0~360） |
| `g_calang_err_deg` | volatile int32_t | 快照：meas−target 折叠到 (−180,180] |
...
## 打印规范（[CALANG] 前缀，.h 内 0/1 开关，整数 deg，无浮点/中文）

- `BETA done -> ALPHA 0deg`
- `LOCKED offset=%d deg`
- `HOLD ok target=%d meas=%d err=%d deg`
- `HOLD FAIL moved=%d cnts target=%d meas=%d deg`
- `FAULT_OC i=%d mA`
...
1. 渐进 90/180/270/0：`g_foc_mech_deg` 每步 +9°，四步共 36°
2. 大跳变：0° 直接设 270° → 转子倒退到 351°
3. HOLD ok 时 err≈0；手扳转子 → FAIL（moved 超阈值）
4. VOFA 看 `g_calang_du/dv/dw` 三相正弦、互差 120°、随输入角变化
5. mode 20 回归不受影响
