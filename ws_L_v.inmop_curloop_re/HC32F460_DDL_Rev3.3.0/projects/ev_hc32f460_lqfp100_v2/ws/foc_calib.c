 ...
 *
 *        平均 4000 样本（= 200ms @20kHz，见 FOC_CALIB_AVG_SAMPLES）：宽带噪声按
 *        sqrt(N) 压缩约 63 倍，0.3A rms 噪声下偏移估计误差 ~5mA rms，
 *        远小于待修正的数百 mA 温漂。
 *        ⚠ 样本数在编译期固定，改 MOTOR_PWM_FREQ_HZ 只改变等效时间长度
 *          （4000 拍 @20kHz = 200ms；@10kHz 则为 400ms）。
 *******************************************************************************
 */
