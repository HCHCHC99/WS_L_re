...
#define FOC45_ISR_DT_US     (1000000u / FOC_ISR_HZ)
#define FOC45_SPD_WIN_TICKS (FOC45_SPD_WIN_MS * FOC_ISR_HZ / 1000u)
#define FOC45_RAD2DEG       57.2958f

volatile uint8_t  g_smo45_running           = 0u;
volatile uint8_t  g_smo45_state             = FOC45_STEP_IDLE;
volatile uint8_t  g_smo45_evt               = 0u;
volatile uint8_t  g_smo45_auto_ramp         = FOC45_AUTO_RAMP_DEFAULT;
...
volatile float    g_smo45_iq_filt_alpha     = FOC45_IQ_FILT_ALPHA;
...
pid_config_t g_smo45_pid_speed_cfg = {
    ...
    .i_valid      = true,
    .d_valid      = false,
    .kp           = FOC45_SPD_KP_MA_PER_RPM,
    .ki           = FOC45_SPD_KI_MA_PER_RPM_S,
    .kd           = 0.0f,
    .output_min   = -FOC45_SPD_IQ_LIMIT_MA,
    .output_max   =  FOC45_SPD_IQ_LIMIT_MA,
    .integral_max =  FOC45_SPD_IQ_LIMIT_MA,
    .i_term_max   =  FOC45_SPD_IQ_LIMIT_MA,
    .update_ms    = 0u,
};
...
pid_config_t g_smo45_pid_id_cfg = {
    ...
    .ki           = FOC45_PI_KI,
    .kd           = 0.0f,
    .output_min   = -FOC45_PI_UMAX_V,
    .output_max   =  FOC45_PI_UMAX_V,
    .integral_max =  FOC45_ITERM_MAX_V,
    .i_term_max   =  FOC45_ITERM_MAX_V,
    .update_ms    = 0u,
};
...
static const uint16_t s_auto_rpm[FOC45_AUTO_RAMP_SECS + 1u] = {
    0u, 200u, 500u, 1000u
};
...
{
    if (rpm > FOC45_SPEED_REF_LIMIT_RPM) {
        rpm = FOC45_SPEED_REF_LIMIT_RPM;
    }
    if (rpm < -FOC45_SPEED_REF_LIMIT_RPM) {
        rpm = -FOC45_SPEED_REF_LIMIT_RPM;
    }
...
static void Smo45_UpdateSpeed(int32_t corrected_delta)
{
    ...
    if (++s_speed_win_tick < FOC45_SPD_WIN_TICKS) {
        return;
    }

    raw_rpm = (float)s_speed_acc_cnt * 60.0f
            * (1000.0f / (float)FOC45_SPD_WIN_MS)
            / (float)ENCODER_CPR;
    if (s_speed_filt_init == 0u) {
        s_speed_filt_rpm = raw_rpm;
        s_speed_filt_init = 1u;
    } else {
        s_speed_filt_rpm += FOC45_SPD_FILT_ALPHA
    ...
    if (s_speed_disp_init == 0u) {
        ...
        s_speed_disp_rpm += FOC45_SPD_DISP_ALPHA
...
static void Smo45_UpdateOuterLoop(void)
{
    ...
    ramp_step = FOC45_ACCEL_LIMIT_RPM_S
              * ((float)FOC45_SPD_WIN_MS * 0.001f);
...
void Foc_Smo45_Start(void)
{
    ...
    if (Foc_Dcal_GetResult(&calibration) == 0u) {
        g_smo45_running = 0u;
        g_smo45_state = FOC45_STEP_IDLE;
        FOC45_LOG("ERROR: no mode 24 calibration; run mode 24 first");
        return;
    }
    ...
    g_smo45_running = 1u;
    g_smo45_state = FOC45_STEP_RUN;
    Foc_Core_PwmStart();
    FOC45_LOG("start rot=%d cnt limit=%.0frpm accel=%.0frpm/s iq=%.0fmA auto=%u",
              (int)s_rotor_count,
              (double)FOC45_SPEED_REF_LIMIT_RPM,
              (double)FOC45_ACCEL_LIMIT_RPM_S,
              (double)FOC45_SPD_IQ_LIMIT_MA,
...
void Foc_Smo45_Step(const stc_i_data_t *pData)
{
    ...
    if (Foc_Core_OverCurrent(pData)) {
        g_smo45_state = FOC45_STEP_FAULT_OC;
        g_smo45_evt = FOC45_EVT_OC;
        g_smo45_running = 0u;
    ...
    if (g_smo45_state != FOC45_STEP_RUN) {
        return;
    }
    ...
    hardware_delta = (int32_t)(int16_t)((uint16_t)hardware_count
                                        - s_encoder_prev_hw);
    s_encoder_prev_hw = hardware_count;
    if (hardware_delta > FOC45_ENC_DELTA_MAX) {
        hardware_delta = FOC45_ENC_DELTA_MAX;
    }
    if (hardware_delta < -FOC45_ENC_DELTA_MAX) {
        hardware_delta = -FOC45_ENC_DELTA_MAX;
    }
    ...
    if (g_smo45_ang_lead_ticks > 0.0f && g_smo45_sl_active == 0u) {
        ...
        rotor_rad += w_e * FOC45_ISR_DT_US * 1.0e-6f * g_smo45_ang_lead_ticks;
        rotor_rad -= (float)((int32_t)(rotor_rad * (1.0f / FOC_MATH_2PI)))
                   * FOC_MATH_2PI;
        if (rotor_rad < 0.0f) rotor_rad += FOC_MATH_2PI;
    }
    ...
    g_smo45_vd = vd;
    g_smo45_vq = vq;
    g_smo45_vsat = ((vd <= -FOC45_PI_UMAX_V + 0.01f) ||
                    (vd >=  FOC45_PI_UMAX_V - 0.01f) ||
                    (vq <= -FOC45_PI_UMAX_V + 0.01f) ||
                    (vq >=  FOC45_PI_UMAX_V - 0.01f)) ? 1u : 0u;
    ...
    Foc_Pll_Compare(rotor_rad * FOC45_RAD2DEG);
    ...
    rotor_deg = (int32_t)(rotor_rad * FOC45_RAD2DEG);
